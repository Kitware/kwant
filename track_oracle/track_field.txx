...
get_element_store( const field_handle_type& fh )
{
  ...
  {
    // whoops
    element_descriptor ed = track_oracle::get_element_descriptor( fh );
...
::lookup_or_create_element_store( const string& field_name )
{
  ...
  field_handle_type f = track_oracle::lookup_by_name( field_name );

  if ( f == INVALID_FIELD_HANDLE )
  {
    // no column by that name; create one
    f = track_oracle::create_element<Type>(
                       element_descriptor(
                         field_name,
                         "no description",
                         my_typeid_str,
  ...
  element_descriptor e = track_oracle::get_element_descriptor( f );
...
{
  return track_oracle::get_field<Type>( row_handle, field_handle );
}
...
::operator()( void ) const
{
  ...
  if ( ! track_oracle::field_has_row( this->host->get_cursor(), field_handle ))
  {
    ostringstream oss;
    oss << "Attempt to read uninitialized field " << this->name << "\n";
    throw runtime_error( oss.str() );
  }
  return track_oracle::get_field<Type>( this->host->get_cursor(), field_handle );
}
...
{
  track_oracle::remove_field<typename track_field<T>::Type>( row, this->field_handle );
}
...
{
  return track_oracle::lookup<Type>( this->field_handle, val, domain );
}
...
{
  frame_handle_list_type frames = track_oracle::get_frames( src_track );

  // just do a linear search for now
  for (unsigned i=0; i<frames.size(); ++i)
  {
    if ( (track_oracle::field_has_row( frames[i].row, this->field_handle)) &&
         (track_oracle::get_field<Type>( frames[i].row, this->field_handle) == val) )
    {
      return frames[i].row;
    }
  }
...
{
  return track_oracle::field_has_row( row, this->field_handle );
}
...
::exists( void ) const
{
  ...
  return track_oracle::field_has_row( this->host->get_cursor(), this->field_handle );
}
...
{
  return track_oracle::get<Type>( row, this->field_handle );
}
...
{
  pair< bool, typename track_field<T>::Type > probe = track_oracle::get<Type>( src, field_handle );
  if ( probe.first )
  {
    track_oracle::get_field<Type>( dst, field_handle ) = probe.second;
  }
}
...
{
  track_oracle::apply_functor( this->field_handle, f );
