                     ...
                     size_t n_frames_per_track )
{
  ...
  for (schema_map_cit i = s.begin(); i != s.end(); ++i)
  {
    ...
    {
      // skip system headers
      if (track_oracle::get_element_descriptor( j->first ).role == element_descriptor::SYSTEM) continue;
  ...
  all_elements[ track_oracle::lookup_by_name( dt::tracking::external_id::c.name ) ] = track_base_impl::IN_TRACK;
  all_elements[ track_oracle::lookup_by_name( dt::utility::state_flags::c.name ) ] = track_base_impl::IN_FRAME;
  all_elements[ track_oracle::lookup_by_name( dt::tracking::frame_number::c.name) ] = track_base_impl::IN_FRAME;
  all_elements[ track_oracle::lookup_by_name( dt::tracking::timestamp_usecs::c.name) ] = track_base_impl::IN_FRAME;
  all_elements[ track_oracle::lookup_by_name( dt::tracking::time_stamp::c.name) ] = track_base_impl::IN_FRAME;
  ...
  for (size_t track_i = 0; track_i<n_tracks; ++track_i)
  {
    ...
    for (size_t i=0; i<track_elements.size(); ++i)
    {
      ...
      {
        track_oracle::get_mutable_element_store_base( fh )->set_to_default_value( t.row );
