...
::list_missing_elements( const oracle_entry_handle_type& row ) const
{
  ...
  for (unsigned i=0; i<this->field_list.size(); ++i)
  {
    ...
    if (! track_oracle::field_has_row(row, f ))
    {
...
{
  return this->contains_element( track_oracle::lookup_by_name( e.name ));
}
...
{
  field_handle_type f = track_oracle::lookup_by_name( e.name );
