...
::create()
{
  oracle_entry_handle_type new_row = track_oracle::get_next_handle();
...
::remove_me()
{
  ...
  vector<frame_handle_type> frames = track_oracle::get_frames( track_handle_type( this->Track.get_cursor() ) );
...
{
  frame_handle_list_type these_frames = track_oracle::get_frames( track_handle_type( this->Track.get_cursor() ) );
  these_frames.insert( these_frames.end(), frames.begin(), frames.end() );
  track_oracle::set_frames( track_handle_type( this->Track.get_cursor() ), these_frames );
}
...
::is_complete( const track_handle_type& row ) const
{
  ...
  const frame_handle_list_type frames = track_oracle::get_frames( row );
  for (unsigned i=0; i<frames.size(); ++i)
  {
...
operator<<( ostream& os, track_base_impl& track )
{
  ...
  frame_handle_list_type frame_list = track_oracle::get_frames( track_handle_type( track.Track.get_cursor() ) );
...
::list_missing_elements( const track_handle_type& _track ) const
{
  ...
  frame_handle_list_type frame_list = track_oracle::get_frames( _track );
  for (unsigned i=0; i<frame_list.size(); ++i)
  {
