...
::scorable_mgrs( const geographic::geo_coords& g )
{
  ...
  this->zone[ z ] = g.zone();
  this->northing[ z ] = g.northing();
  this->easting[ z ] = g.easting();
  this->entry_valid[ z ] = true;
  z = ALT_ZONE;
  if ( g.zone() != g.alt_zone() )
  {
    this->zone[ z ] = g.alt_zone();
    this->northing[ z ] = g.alt_northing();
    this->easting[ z ] = g.alt_easting();
    this->entry_valid[ z ] = true;
  }
  else
  {
    this->mark_zone_invalid( z );
  }
