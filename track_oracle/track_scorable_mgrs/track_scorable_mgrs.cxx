                      ...
                      const string& lat_field_name )
{
  ...
  for (size_t i=0; i<tracks.size(); ++i)
  {
    ...
    const frame_handle_list_type& frames = track_oracle::get_frames( t );
    ...
    for (size_t f_index=0; f_index<frames.size(); ++f_index)
    {
      ...
      default:
        ...
        {
          LOG_ERROR( "Couldn't find latitude and/or longitude field to set MGRS (radial overlap) info-- format is "
        ...
        if ( first_time )
        {
          LOG_INFO( "Using " << file_format_type::to_string( tf ) << " fields '" << lon_field_name
